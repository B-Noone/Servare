...
public class Grid : MonoBehaviour {
    ...
    public Nodes NodeWorldPoint(Vector3 worldPosition)
    {
    ...
    public List<Nodes> GetAdjacent(Nodes node)
    {
        List<Nodes> adjacent = new List<Nodes>();
        for(int x = -1; x <= 1; x++)
        {
            for (int y = -1; y <= 1; y++)
            {
    ...
    void OnDrawGizmos()
    {
        ...
        {
            foreach (Nodes i in grid)
            {
                Gizmos.color = (i.walkable) ? Color.white : Color.red;
                Gizmos.DrawCube(i.worldPos, Vector3.one * (nodeDiameter - 0.1f));
            }
        }
    }
}
